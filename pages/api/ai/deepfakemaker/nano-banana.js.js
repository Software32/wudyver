...
class DeepFakeAPI {
  ...
  async generate({
    ...
    console.log(`\nMemulai tugas [Banana] dengan prompt: "${prompt}"`);
    try {
      ...
      console.log("Mengirim permintaan untuk membuat tugas [Banana]...");
      const payload = {
        prompt: prompt,
      ...
      const response = await axios.post(`${API_BASE_URL}/replicate/v1/free/nano/banana/task`, payload, {
        params: this._getAuthParams()
      });
      const resultGen = response?.data || null;
      return resultGen;
    } catch (error) {
      console.error("Gagal membuat tugas [Banana]:", error?.response?.data || error.message);
      return null;
    }
  }
  async status({
    ...
    console.log(`\nMemeriksa status tugas [Banana] untuk ID: ${task_id}`);
    if (!task_id) {
      console.error("ID tugas [Banana] tidak disediakan.");
      return null;
    }
    try {
      const requestParams = {
      ...
      const response = await axios.get(`${API_BASE_URL}/replicate/v1/free/nano/banana/task`, {
        params: requestParams
      });
      const resultTask = response?.data || null;
      return resultTask;
    } catch (error) {
      console.error("Gagal memeriksa status tugas [Banana]:", error?.response?.data || error.message);
      return null;
    }
  }
}
